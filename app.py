...
if 'all_batting_data' not in st.session_state:
    st.session_state.all_batting_data = pd.DataFrame(columns=[
        'チーム名', '選手名', '打席ID', 'コース', '球種', '捕球位置', '打球角度', '投手の投げ手', 'カウント', '結果', '入力日時'
    ])
...
if team_name and player_name:
    ...
    # --- データのエクスポート（Streamlit上でファイル保存は直接できないため、ダウンロードリンクを提供） --- #
    st.markdown("### データの保存と整理")
    st.write("Streamlitアプリケーションはサーバサイドで動作するため、直接クライアントのファイルシステムに保存することはできません。")
    st.write("しかし、以下の方法でデータをダウンロードし、ローカルで整理することは可能です。")
    st.write("**データ整理の概念:** チーム名/選手名ごとにCSVファイルとして保存することを想定します。例えば、`チームA/選手1.csv` のように。")

    if not st.session_state.all_batting_data.empty:
        # 全データをダウンロード
        csv_all = st.session_state.all_batting_data.to_csv(index=False).encode('utf-8')
        st.download_button(
            label="全打席データをCSVでダウンロード",
            data=csv_all,
            file_name="all_batting_data.csv",
            mime="text/csv",
            key='download_all'
        )
        ...
        if player_name and team_name:
            player_data_to_save = st.session_state.all_batting_data[
                (st.session_state.all_batting_data['チーム名'] == team_name) &
                (st.session_state.all_batting_data['選手名'] == player_name)
            ]
            if not player_data_to_save.empty:
                csv_player = player_data_to_save.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label=f"{team_name} - {player_name} のデータをCSVでダウンロード",
                    data=csv_player,
                    file_name=f"{team_name}_{player_name}_batting_data.csv",
                    mime="text/csv",
                    key='download_player'
                )
            else:
                st.write(f"{team_name} - {player_name} のダウンロード可能なデータはありません。")

    st.write("**メモ:** Colab上でStreamlitを停止するには、ターミナルでCtrl+Cを押してください。")
